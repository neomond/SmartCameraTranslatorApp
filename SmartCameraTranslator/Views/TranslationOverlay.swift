...
struct TranslationOverlay: View {
    ...
    var body: some View {
        ZStack {
            ForEach(detectedTexts) { detection in
                ...
                // Always show detected text label with smart positioning
                Text(detection.text)
                    ...
                    .position(
                        x: rect.midX,
                        y: smartLabelPosition(for: rect, in: geometrySize)
                    )
                ...
                if selectedText?.id == detection.id {
                    TranslationBubble(
                        ...
                        position: smartBubblePosition(for: rect, in: geometrySize),
    ...
    // Smart positioning for text labels
    private func smartLabelPosition(for rect: CGRect, in geometry: CGSize) -> CGFloat {
        let safeAreaTop: CGFloat = 100 // Account for status bar, notch, and UI controls
        let labelHeight: CGFloat = 30
        
        // Try to position above the bounding box
        let preferredY = rect.minY - 20
        
        // If it would be cut off at the top, position below the box
        if preferredY < safeAreaTop {
            return min(rect.maxY + 20, geometry.height - labelHeight)
        }
        
        return preferredY
    }
    
    // Smart positioning for translation bubbles
    private func smartBubblePosition(for rect: CGRect, in geometry: CGSize) -> CGPoint {
        let safeAreaTop: CGFloat = 120
        let bubbleHeight: CGFloat = 100
        let x = rect.midX
        
        // Try to position above the bounding box
        let preferredY = rect.minY - 60
        
        // If it would be cut off at the top, position below the box
        let y = preferredY < safeAreaTop ?
            min(rect.maxY + 60, geometry.height - bubbleHeight) :
            preferredY
